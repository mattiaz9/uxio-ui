...
`lib/duration-format.ts`). If a larger unit is absent from the pattern, the smaller segment does not
carry upward.
...
export function DurationFormatField() {
  ...
  return <InputDuration format="dd'd' HH'h' mm'm'" value={value} onValueChange={setValue} />
}
```
...
export function ColonDurationField() {
  ...
  return <InputDuration format="HH:mm:ss" value={value} onValueChange={setValue} />
}
```
