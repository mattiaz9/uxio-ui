...
- [`AutoTooltip`](#auto-mode) composes trigger and content. With `mode="auto"`, it opens only
  when the trigger is truncated (overflow hidden / ellipsis).
...
The tooltip appears only when the label is truncated. A second line that fully fits does not open
a tooltip.
