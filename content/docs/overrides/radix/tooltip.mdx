...
- [`AutoTooltip`](#auto-mode) composes trigger and content, and with `mode="auto"` only opens when
  the trigger is truncated (overflow hidden / ellipsis).
...
For Radix, wrap your app (or a subtree) with `TooltipProvider` from the same module when you use the
low-level `Tooltip` / `TooltipTrigger` / `TooltipContent` primitives so delay and “skip delay”
behavior work across tooltips. `AutoTooltip` wraps an inner provider for its own delay.
...
The tooltip appears only when the label is truncated. Below it, a short line that fully fits does
not open a tooltip.

<ComponentPreview name="radix/tooltip-auto" />

### Always mode

The tooltip can open even when the full trigger text fits.

<ComponentPreview name="radix/tooltip-always" />
