...
`AutoTooltip` wraps a single trigger element and supports truncation-based **`mode`**, **`content`** / **`htmlContent`**, **`delay`**, **`placement`**, **`offset`**, **`appendTo`**, and **`className`** on the content.
