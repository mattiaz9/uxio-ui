...
Use `confirm()` to show a confirmation and optionally run an async action.
...
```bash
npx shadcn@latest add @uxio/confirmation
```
...
void confirm({
  ...
  action: async () => {
    ...
    if (result.data) {
      router.replace(route("/:org/:project", { org, project }))
    } else {
...
<ComponentPreview name="radix/confirmation-return" />

### Action with loading

<ComponentPreview name="radix/confirmation-action" />
