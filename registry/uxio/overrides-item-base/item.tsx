...
const itemGroupVariants = cva("cn-item-group group/item-group flex w-full flex-col", {
  variants: {
    variant: {
      default: "gap-4 has-data-[size=sm]:gap-2.5 has-data-[size=xs]:gap-2",
      grouped:
        "*:not-first:border-t-0 *:not-first:not-last:rounded-none *:first:not-last:rounded-b-none *:last:not-first:rounded-t-none",
    },
  },
  defaultVariants: {
    variant: "default",
...
function ItemSeparator({ className, ...props }: SeparatorPrimitive.Props) {
  return (
    <SeparatorPrimitive
      data-slot="item-separator"
      orientation="horizontal"
      className={cn(
        "cn-item-separator shrink-0 bg-border data-horizontal:h-px data-horizontal:w-full data-vertical:w-px data-vertical:self-stretch",
        className,
      )}
      {...props}
    />
