...
Import them as `@/registry/uxio/shared/{name}/…`; `scripts/build-registry.ts` merges them into
any published item that references them.

Docs-only hooks under `src/app/(create)/lib/` (e.g. `search-params.ts`) are bundled the same way when
a registry file imports `@/app/(create)/lib/…`, and those imports are rewritten to `@/components/ui/…`
in published JSON.
